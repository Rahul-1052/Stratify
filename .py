...
def safe_float(value):
    try:
        # Older cached Stratify runs may store viral_score as a diagnostics dictionary.
        # Always extract the actual numeric score before displaying or classifying it.
        if isinstance(value, dict):
            for key in ["overall_viral_score", "viral_score", "score"]:
                if key in value:
...
def get_viral_score_value(video_or_value):
    """Return only the numeric viral score, even if old cached data stored a dict."""
    if isinstance(video_or_value, dict) and "viral_score" in video_or_value:
        return round(safe_float(video_or_value.get("viral_score", 0)), 1)
    return round(safe_float(video_or_value), 1)


def format_viral_score(video_or_value):
    return f"{get_viral_score_value(video_or_value)}/100"
...
def build_video_executive_summary(video):
    ...
    return [
        ...
        f"Viral potential: {format_viral_score(viral_score)} ({viral_label}).",
...
def calculate_growth_diagnosis(channel, videos, scorecard):
    """Diagnose the biggest channel growth blocker from recent video signals."""
    if not videos:
        return {
            "primary_blocker": "Not enough recent video data",
            "confidence": 0,
            "impact": "Unknown",
            "why_it_matters": "Stratify needs recent uploads to compare consistency, engagement, momentum, packaging, and audience pull.",
            "quick_fix": "Collect enough recent uploads before making a strategy change.",
            "evidence": ["No recent videos were available for diagnosis."],
            "action_plan": ["Analyze at least 5 recent videos before making channel-level decisions."],
            "seven_day_plan": [
                "Day 1: Add 5-10 recent uploads to create a reliable baseline.",
                "Day 2-3: Identify the strongest and weakest recent uploads.",
                "Day 4-5: Compare titles, thumbnails, and topics for pattern differences.",
                "Day 6-7: Make one controlled improvement before the next upload."
            ],
            "avoid": "Avoid changing your whole strategy from one upload.",
            "blockers": []
        }
    ...
    def add_check(name, score, threshold, impact, why, evidence, action):
        gap = max(threshold - safe_float(score), 0)
        severity = min(round((gap / max(threshold, 1)) * 100, 1), 100)
        if gap > 0:
            checks.append({
                "blocker": name,
                "score": round(safe_float(score), 1),
                "threshold": threshold,
                "severity": severity,
                "impact": impact,
                "why": why,
    ...
    add_check(
        "Weak Packaging",
        scorecard.get("optimization_score", 0),
        58,
        "High",
        "Viewers may not clearly understand why they should click before they scroll away.",
        f"Packaging / Optimization score is {scorecard.get('optimization_score', 0)}/100. Average recent performance score is {round(avg_score, 1)}/100.",
        "Improve title clarity, thumbnail focus, and the first visible promise of each upload before increasing volume."
    )

    add_check(
        "Weak Audience Engagement",
        scorecard.get("engagement_score", 0),
        55,
        "High",
    ...
    add_check(
        ...
        scorecard.get("consistency_score", 0),
        50,
        "Medium",
        "Inconsistent posting makes it harder for viewers to build a habit around the channel.",
        f"Consistency score is {scorecard.get('consistency_score', 0)}/100 across the analyzed uploads.",
        "Create a realistic weekly rhythm and repeat proven formats instead of randomly uploading unrelated ideas."
    )

    add_check(
        "Low Audience Pull",
        scorecard.get("audience_pull_score", 0),
        45,
        "High",
    ...
    add_check(
        ...
        scorecard.get("momentum_score", 0),
        50,
        "Medium",
    ...
    add_check(
        "Low Community Conversation",
        scorecard.get("content_depth_score", 0),
        35,
        "Medium",
    ...
    impact_weight = {"High": 1.25, "Medium": 1.0, "Low": 0.75}
    checks = sorted(checks, key=lambda item: item["severity"] * impact_weight.get(item["impact"], 1), reverse=True)

    if not checks:
        top_video = get_top_video_by_score(videos)
        top_title = top_video.get("title", "your strongest recent upload") if top_video else "your strongest recent upload"
        return {
            "primary_blocker": "No major growth blocker detected",
            "confidence": 78,
            "impact": "Opportunity",
            "why_it_matters": "The channel has a healthy baseline. The next move is not fixing a weakness; it is scaling what already works.",
            "quick_fix": "Scale the strongest recent format before testing unrelated ideas.",
            "evidence": [
                f"Creator Score is {scorecard.get('overall_score', 0)}/100 with no major weak category below the diagnosis thresholds.",
                f"Strongest recent signal: {top_title}."
            ],
            "action_plan": [
                "Turn the strongest recent upload into a repeatable series or format.",
                "Create 2-3 variations of the same winning idea before testing unrelated topics.",
                "Track whether the repeated format beats the channel average for views and engagement."
            ],
            "seven_day_plan": [
                "Day 1: Pick the strongest recent upload and identify its format.",
                "Day 2: Write three follow-up ideas using the same viewer promise.",
                "Day 3-4: Build titles and thumbnails around the clearest payoff.",
                "Day 5-7: Publish or prepare the best variation and compare it to the channel average."
            ],
            "avoid": "Avoid changing too many variables at once when the current system is already working.",
            "blockers": []
        }

    primary = checks[0]
    confidence = round(min(92, max(62, 60 + primary["severity"] * 0.45 + (8 if primary["impact"] == "High" else 0))), 1)

    action_plan = [primary["action"]]
    for item in checks[1:3]:
        action_plan.append(item["action"])

    blocker = primary["blocker"]
    quick_fix_map = {
        "Weak Packaging": "Fix the title and thumbnail promise before uploading more videos.",
        "Weak Audience Engagement": "Add a stronger emotional payoff and a natural reason for viewers to react.",
        "Inconsistent Upload Rhythm": "Create a repeatable weekly upload rhythm using one proven format.",
        "Low Audience Pull": "Double down on topics that already beat your channel average.",
        "Losing Recent Momentum": "Bring back the older formats that clearly had stronger demand.",
        "Low Community Conversation": "Turn passive viewers into commenters with sharper debate prompts."
    }
    seven_day_map = {
        "Weak Packaging": [
            "Day 1: List the top 5 and bottom 5 recent uploads by performance.",
            "Day 2: Compare their titles and thumbnails for clarity, emotion, and curiosity.",
            "Day 3-4: Rewrite 5 weak titles with a clearer payoff in the first few words.",
            "Day 5-6: Redesign one thumbnail style around one focal point and stronger contrast.",
            "Day 7: Use the new packaging on the next upload and compare results to the channel average."
        ],
        "Weak Audience Engagement": [
            "Day 1: Identify where the strongest emotional moments happen in top videos.",
            "Day 2: Add one audience question that invites opinions, not yes/no answers.",
            "Day 3-4: Plan a stronger opening payoff within the first 15 seconds.",
            "Day 5-6: Add one shareable moment, reveal, joke, debate, or reaction point.",
            "Day 7: Pin a comment that starts a conversation around the main moment."
        ],
        "Inconsistent Upload Rhythm": [
            "Day 1: Pick one realistic weekly upload day.",
            "Day 2: Choose one repeatable format from recent winners.",
            "Day 3-5: Batch 2-3 ideas in that same format.",
            "Day 6: Prepare titles and thumbnails before recording or editing.",
            "Day 7: Lock the next 2 uploads into a simple content calendar."
        ],
        "Low Audience Pull": [
            "Day 1: Find videos that beat average views by at least 1.5x.",
            "Day 2: Identify the topic, character, format, or promise they share.",
            "Day 3-4: Create 3 new ideas using that same demand signal.",
            "Day 5-6: Make the viewer payoff more obvious in title and thumbnail.",
            "Day 7: Prioritize the idea closest to an existing winner."
        ],
        "Losing Recent Momentum": [
            "Day 1: Compare recent uploads against older stronger uploads.",
            "Day 2: Identify what changed in topic, format, pacing, or packaging.",
            "Day 3-4: Bring back one older winning format with a fresh angle.",
            "Day 5-6: Avoid unrelated experiments until momentum stabilizes.",
            "Day 7: Publish or prepare one comeback-format upload."
        ],
        "Low Community Conversation": [
            "Day 1: Read comments on the top 5 recent uploads and collect repeat themes.",
            "Day 2: Create one debate question around a character, ranking, decision, or opinion.",
            "Day 3-4: Add the debate into the video idea, not only the pinned comment.",
            "Day 5-6: Write a pinned comment that asks viewers to choose a side.",
            "Day 7: Reply to early comments to train the audience to participate."
        ]
    }
    ...
    return {
        ...
        "why_it_matters": primary["why"],
        "quick_fix": quick_fix_map.get(blocker, primary["action"]),
        "evidence": [primary["evidence"]] + [item["evidence"] for item in checks[1:3]],
        "action_plan": action_plan,
        "seven_day_plan": seven_day_map.get(blocker, action_plan),
        "avoid": "Avoid trying to fix everything at once. Fix the highest-impact blocker first, then re-check the next 10-15 uploads.",
        "blockers": checks[:5]
    }
...
def build_video_report(video, insights):
    metrics = {
        ...
        "Viral Score": format_viral_score(video),
    }
...
def build_channel_report(channel, scorecard, videos):
    metrics = {
        ...
        "Community Depth": f"{scorecard.get('content_depth_score', 0)}/100",
    }
    ...
    diagnosis_summary = [
        f"Primary Growth Blocker: {diagnosis.get('primary_blocker', 'Not available')}",
        f"Confidence: {diagnosis.get('confidence', 0)}/100",
        f"Impact: {diagnosis.get('impact', 'Unknown')}",
        f"What to fix first: {diagnosis.get('quick_fix', 'Focus on the highest-impact blocker first.')}",
        f"Why it matters: {diagnosis.get('why_it_matters', 'Not available')}",
    ]
    seven_day_summary = diagnosis.get("seven_day_plan", diagnosis.get("action_plan", []))

    sections = [
        ("Growth Diagnosis", diagnosis_summary),
        ("Next 7-Day Action Plan", seven_day_summary),
...
with tabs[0]:
    ...
    if st.button("Analyze Video", width="stretch"):
        ...
        with c5:
            render_metric_card("Viral Score", format_viral_score(video))
...
with tabs[1]:
    ...
    if st.button("Analyze Channel", width="stretch"):
        ...
        diagnosis = calculate_growth_diagnosis(channel, videos, scorecard)
        render_growth_diagnosis(diagnosis)
...
with tabs[2]:
    ...
    if st.button("Compare Videos", width="stretch"):
        ...
        with c4:
            render_metric_card("Best Viral Score", format_viral_score(best_viral["viral_score"]))
